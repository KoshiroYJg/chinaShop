...
public class UserController {  
    ...
    public String addGoods(Goods goods,HttpSession session,HttpServletRequest request) {
		...
		try {
			goods.setGoodsUrl(FileUtil.shangc(goods.getFile1()));
			goods.setGoodsUrl2(FileUtil.shangc(goods.getFile2()));
			goods.setGoodsUrl3(FileUtil.shangc(goods.getFile3()));
			goods.setGoodsUrl4(FileUtil.shangc(goods.getFile4()));
		} catch (Exception e) {
			returnMap.put("RetCode", "0");
			returnMap.put("RetMessage", "失败");
			String jsonString = JSON.toJSONString(returnMap);
	...
	@ResponseBody
    public String addGoods2(Goods goods,HttpSession session, HttpServletRequest request) {
